...
### Build-time prerender (SEO)

`vite/prerender.ts` is a **build-only** Vite plugin that bakes the whole
526-statement text into `dist/index.html` as a `#prerender` div preceding
`#root`, and emits `sitemap.xml`. It exists because the SPA otherwise ships an
empty div, which every non-JS crawler indexes as a blank page. An inline script
hides the block before first paint and `src/main.tsx` removes it before React
mounts, so the running app is untouched. It reuses `parseStatement` from
`src/lib/math.ts` — **do not add a second text parser**; and it only works
because `src/model/tree.ts` and `src/lib/math.ts` are pure (no DOM, no React, no
`import.meta.env`), since `vite.config.ts` bundles them at config load. The
canonical origin is stated in both `index.html` and `src/lib/site.ts`
(`SITE_URL`) and the plugin fails the build if they disagree. Guarded by
`vite/prerender.test.ts`.

### Fonts

Self-hosted from `public/fonts/`, declared in `src/styles/fonts.css`, both
generated by `scripts/build-fonts.sh` and **committed** — no network at build
time. Material Symbols is subset from 508 KiB to 22 KiB; the subset is keyed on
the font's **ligature** names, not its glyph names (they differ, and using glyph
names silently drops icons like `file_download`). If you add an icon to a
component, re-run the script — `vite/fonts.test.ts` fails otherwise. The FILL
axis must stay variable; `reading.css` animates it between 0 and 1.
